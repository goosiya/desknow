// 인증 실패 → 사용자 카피 매핑 (Story 9.1 — 웹 authCopy.ts 순수 함수 복사, AC2).
//
// 막다른 화면 금지: 모든 분기가 재시도 가능한 인라인 안내다(별도 화면 없음). "오프라인" 금지 —
// 네트워크 단절은 프로젝트 표준 카피로 단일화한다([[terminology-network-disconnect-not-offline]]).
// 웹과 문구·검증 규칙이 100% 동일해야 한다(패리티) — RN 의존성 없는 순수 함수다.
...
/**
 * 가입 자격 클라이언트 1차 검증. provider 가입은 실제 서버 호출(register)이 룸 등록 시점까지
 * 미뤄지므로, 빈값·형식·정책 위반을 **넘어가기 전에** 여기서 거른다(booker는 register가 즉시 서버
 * 검증). 통과(null)면 진행, 아니면 사용자 카피를 반환한다. 최종 신뢰 경계는 서버
 * (`_enforce_password_policy`)이며 이 함수는 그 규칙을 미러한 1차 가드일 뿐이다.
 */
