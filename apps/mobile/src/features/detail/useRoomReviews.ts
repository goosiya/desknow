// 룸 상세 후기 목록 쿼리 — 웹 detail/useRoomReviews.ts 미러 (Story 9.2 — AC1). 룸 상세 3차 섹션이
// 후기를 한 곳에서 본다.
//
...
//    요약 배지를 불필요하게 재조회시키지 않는다(정확 키 — favorites 선례).
//
// 백엔드 호출은 생성 SDK 경유만(직접 fetch 금지 — 1.9 가드). reviewsListRoomReviews는 공개·무인증
// 엔드포인트라 인증 불요. roomId 가 빈 문자열이면 비활성(상세 미선택).
 ...
 *
 * **무한스크롤:** 커서 페이징(`useInfiniteQuery`) — `select` 평탄화로 `data`는 `ReviewListItem[]`.
 */
