// 즉시 예약 확정 뮤테이션 — 웹 reservation/useCreateReservation.ts 미러 (Story 9.2 — AC3).
//
// `useMutation` + `onSuccess` 에서 **정확 키만** invalidate(광역 금지). **옵티미스틱은 적용하지
// 않는다**: 예약은 서버 확정이 진실이라(충돌 가능) 응답 성공 후에만 성공 표시한다(architecture.md
// L277 옵티미스틱 경계).
//
// 백엔드 호출은 생성 SDK 경유만(직접 fetch 금지 — 1.9 가드). 인증 헤더(Bearer)는 api-client.ts
// 인터셉터가 주입한다(booker 세션).
...
/**
 * 즉시 예약 확정 뮤테이션(AC3). 성공 시 정확 키만 invalidate(광역 금지 — useFavorites 선례).
 *
 ...
 *   invalidate한다. `["rooms"]` 광역 무효화 금지.
 * - `onError`: **`SLOT_CONFLICT`(409)일 때만** 그날 슬롯 쿼리를 invalidate해 `SlotGrid`를 재조회한다
 *   (= 인접 빈 슬롯 즉시 재표시 — AC3). generic 실패(404·5xx 등)는 재조회 불요(selection 유지·재시도).
...
export function useCreateReservation(roomId: string) {
  ...
  return useMutation({
    ...
    onError: (error, { selectedDate }) => {
      // SLOT_CONFLICT 일 때만 그날 슬롯을 재조회(인접 빈 슬롯 재표시). 정확 키만.
