// 바텀시트 요약 순수 로직 (Story 3.3 — 프레임워크 무관 · 테스트·재사용 단일 출처).
//
// 카카오/DOM/React 에 의존하지 않는 순수 함수만 둔다. 가격·영업시간 포맷, 오늘 요일 행 선택,
// 부대시설/룸형태 라벨 매핑을 담당한다.
//
// ⚠️ 금지 안티패턴(architecture.md L293·L362): **클라가 슬롯을 재계산하지 않는다.**
//    todayBusinessHours 는 "오늘 요일 행을 고르는 날짜 포맷"일 뿐 슬롯 도출이 아니다(슬롯은
//    서버 derive_slots 가 1회 끝낸 remaining_slots 가 끝낸다). business_hours 를 슬롯으로
//    환산하는 코드를 절대 작성하지 않는다.
...
// KST(Asia/Seoul) 요일 약어 → date.weekday() 규약(월=0 … 일=6) 매핑. 서버 business_hours.weekday
// 와 같은 규약으로 맞춘다(섞이면 다른 요일 행을 표시하게 됨).
 ...
 *    remaining_slots(summaryStatus)가 결정한다(anti-pattern 회피). now 는 테스트 결정성을 위해
 *    주입받으며, 미지정 시 현재시각을 쓴다(프로세스 로컬 tz 가 아니라 Asia/Seoul 로 요일 판정).
 */
...
/** 신선 remaining_slots 로 예약 가능/마감을 도출한다 — pin.ts 의 pinStatus 재사용(중복 금지). */
