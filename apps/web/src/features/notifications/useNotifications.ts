// 인앱 통지 쿼리/소멸 뮤테이션 (Story 5.1 — AC3·AC5). 전역 배너가 소비하는 영속 통지 단일 출처.
//
...
// (직접 fetch 금지 — 1.9 가드). credentials:"include" 는 api-client 전역(로그인 세션).
...
/**
 * 통지 소멸 뮤테이션(옵티미스틱 — AC5). '다시 보지 않기'·'확인' 공통 진입점(단일 generic dismiss).
 *
 * **옵티미스틱 패턴(useToggleFavorite 선례):**
 * - onMutate: cancelQueries → 스냅샷 previous → 해당 id 제거로 즉시 반영(≤100ms 로컬 — 배너 사라짐).
 * - onError: previous 로 롤백(서버 실패 시 배너 복원).
 * - onSettled: ["notifications"] **정확 키만** invalidate(절대 ["rooms"]/["reservations"] 광역 금지).
 *
 * 소멸은 서버에 영속(dismissed_at)되므로 재접속 시 다시 뜨지 않는다(AC5). 멱등(서버가 보장)이라
 * 중복 클릭·재시도에도 안전하다.
 */
...
/**
 * 도래 리마인드 '다시 보지 않기' 뮤테이션(옵티미스틱 — Story 5.2 AC2·AC4).
 *
 ...
 * 트리거). onError 롤백. onSettled는 ["notifications"] **정확 키만** invalidate(광역 ["rooms"]/
 * ["reservations"] 금지 — 반복함정 #1). 멱등(서버 보장)이라 재클릭·재시도 안전.
 */
