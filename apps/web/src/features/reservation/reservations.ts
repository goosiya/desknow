...
/**
 * 다가오는 예약인가 — `confirmed` 이고 예약 종료(마지막 슬롯 +1h)가 아직 안 지났는가(AC1).
 *
...
/**
 * 취소 가능한가 — `confirmed` 이고 가장 이른 슬롯 시작까지 6시간 이상 남았는가(AC2 — 4.7 위임).
 *
 * `earliest - now >= 6h`(순수 duration — 두 UTC 인스턴트 차, ROOM_TZ 불요, 4.7 범위 결정 #3).
 * 취소/거절/이용 완료(종료·과거)는 미cancellable. 서버가 6h 경계를 timedelta 로 최종 강제하므로
 * (클럭 스큐 graceful) 이 FE 계산은 버튼 활성/비활성 표시용이다(AC2).
 */
...
/**
 * 예약 시간 범위 라벨 "14:00–17:00"(첫 슬롯 시작 ~ 마지막 슬롯 시작 +1h, KST 벽시계, AC1).
 *
 * `selectionLabels.rangeLabel` 과 동형(formatSlotLabel + 끝슬롯 +1h). 슬롯 0건(레거시)이면 null.
 * 비연속 슬롯도 min..max+1h 범위로 요약한다(스냅샷 전체 보존이라 향후 정밀 표시 가능 — deferred).
 */
