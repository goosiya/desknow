// 예약 흐름(4.3~4.5) 날짜·시간 순수 로직 (Story 4.3 — 프레임워크 무관 · 테스트·재사용 단일 출처).
//
// 카카오/DOM/React 에 의존하지 않는 순수 함수만 둔다. 달력 그리드 계산·선택 가능 날짜 판정·
// 날짜/시간 표시 포맷을 담당한다. 프론트에 `lib/datetime` 이 없으므로(architecture.md L384 의도)
// 이 모듈이 예약 흐름의 날짜 유틸 출처가 된다(feature-local 로 시작 — 후속 흐름이 공유하면 승격).
//
// ⚠️ 금지 안티패턴(architecture.md L362·L293): **클라가 슬롯을 재계산하지 않는다.** 슬롯 리스트·
//    상태는 전부 서버(`/slots`)가 준다. 여기서는 표시 포맷(UTC→KST 벽시계 "14:00")·달력 그리드
//    날짜 산술·선택 가능 날짜 범위 판정만 한다 — 슬롯 가용성 계산은 하지 않는다.
//    Story 4.4 추가분(연속 선택 헬퍼)도 같은 원칙: **렌더된 `slots` 배열의 인덱스 산술**·표시
//    포맷·금액 곱셈만 한다. 가용성 판정은 서버가 준 `slot.status` 를 **읽기만** 한다.
...
/** "YYYY-MM-DD" → "2026년 6월 18일"(달력 셀 aria-label — 스크린리더 한국어, AC2). */
export function formatDateAriaLabel(date: string): string {
  const [year, month, day] = parseYmd(date);
  return `${year}년 ${month}월 ${day}일`;
}

// ── Story 4.4: 연속 슬롯 선택 순수 헬퍼 ──────────────────────────────────────────
//
// 선택은 **렌더된 `slots` 배열의 포함 구간 인덱스**로만 다룬다(시각 산술로 연속성 판정 금지 —
// 배열은 이미 시간순). `clampContiguousAvailable` 하나가 D1(비연속 불허)과 "구간에 점유가 끼면
// 막힘"을 **동시에** 강제한다: 앵커에서 시작해 인접 `available` 슬롯으로만 자라고, 진행 방향 첫
// 비-`available` 슬롯 직전에서 멈춘다 → 떨어진 두 슬롯이 한 구간이 될 수 없다.
...
/**
 * 선택 구간의 **모든 슬롯이 여전히 `available`** 인가(Story 4.9 — stale 선택 무효화, AC5).
 *
 * 배경: 4.9 차감 배선 후 백그라운드 refetch로 슬롯이 `reserved`로 바뀌어도 **인덱스는 안 밀린다**
 * (사라지지 않고 같은 인덱스의 `status`만 변경 — get_room_slots는 reserved를 표시·유지). 그래서
 * 기존 bounds 가드(`safeSelection`)는 통과하지만 선택 구간 내용이 stale인 케이스가 생긴다
 * (deferred L224 "동일 길이·다른 내용"). 이 순수 헬퍼가 선택 구간 `[startIndex, endIndex]`의 모든
 * 슬롯이 `available`인지 확인해, 하나라도 비-`available`(`reserved`/`past`)이면 `false`를 돌려준다
 * (호출처가 선택을 무효화). 슬롯 재계산 아님 — 서버가 준 `slot.status`를 **읽기만** 한다(L362 준수).
 */
...
/**
 * 앵커→대상 방향으로 **첫 비-`available` 슬롯 직전까지** 확장한 연속-가용 구간을 반환한다(AC1·D1).
 *
 * - 앵커가 `available` 가 아니면 `null`(선택 시작 불가).
 * - 대상이 앵커보다 앞이어도(역방향) 대칭 처리한다.
 * - 진행 중 점유 슬롯을 만나면 그 직전에서 멈춘다 → **점유를 못 넘으므로** 비연속·점유 가로지르기가
 *   구조적으로 불가능하다(D1 + 구간 점유 차단을 단일 규칙으로 동시 충족).
 *
 * 반환 구간은 항상 `startIndex <= endIndex` 로 정규화한다(드래그 역방향도 동일 표현).
 */
...
/**
 * 선택 구간 → 점유할 `slot_start[]`(서버 UTC ISO 그대로) — 예약 POST 본문 추출(AC4).
 *
 * **순수 인덱스 슬라이스만** 한다(클라 슬롯 재계산 아님 — architecture.md L362). 렌더된 `slots`
 * 배열에서 `[startIndex, endIndex]` 포함 구간의 `slot_start`(서버가 준 UTC 인스턴트)를 그대로
 * 뽑아 `useCreateReservation` 이 본문에 싣는다. 가용성/연속성은 4.4 헬퍼·서버가 이미 보장했다.
 */
