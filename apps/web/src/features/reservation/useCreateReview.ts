// 후기 작성 뮤테이션 (Story 5.5 — AC1·AC5). 예약현황의 이용 완료 행에서 후기를 작성한다.
//
// useCreateReservation/useCancelReservation 선례 미러 — `useMutation` + onSuccess 에서 **정확
// 키만** invalidate(광역 금지). 옵티미스틱 없음(서버 확정이 진실 — 4.5 선례).
//
// onSuccess: ① ["reservations"](예약현황 — has_review 갱신 → 행이 "후기 완료"로 전환) +
// ② ["rooms", roomId, "reviews"](룸 상세 후기 — 새 후기 즉시 반영). 광역 ["rooms"] 단일키 금지.
// onError: 409(이용 완료 안 됨/이미 작성)는 클럭 스큐·경합이라 ["reservations"] 재조회로 게이팅
// 갱신(graceful). 422·기타는 컴포넌트가 친근 카피로 안내(재조회 불요).
//
...
/**
 * 후기 작성 뮤테이션(AC1·AC5). 성공 시 ["reservations"] + ["rooms", roomId, "reviews"] 정확 invalidate.
 *
...
export function useCreateReview() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      ...
      // 작성 실패라 룸 상세 후기는 무변화 → ["reservations"]만(룸 후기 invalidate 불요).
